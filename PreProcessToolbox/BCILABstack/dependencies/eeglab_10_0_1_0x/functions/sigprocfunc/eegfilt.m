...
%       end
%     end
%% Changed by Rajat - works in parallel
    parfor c=1:chans
        smoothdatamin=smoothdata(c,:);
        datamin=data(c,:);
        try
            smoothdatamin(1,(e-1)*epochframes+1:e*epochframes) ...
                = filtfilt(filtwts,1,datamin(1,(e-1)*epochframes+1:e*epochframes));
        catch,
            smoothdatamin(1,(e-1)*epochframes+1:e*epochframes) ...
                = filtfilt(filtwts,1,double(datamin(1,(e-1)*epochframes+1:e*epochframes)));
        end;
      if epochs == 1 
           if rem(c,20) ~= 0, fprintf('.');
           else               fprintf('%d',c);
